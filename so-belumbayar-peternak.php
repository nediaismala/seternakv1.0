<?php
    ...
    left join public.user on peternak.id_peternak=public.user.username where pemesanan.status='1' and produk.id_peternak='$username';");
                ...
                <ul class="list-group list-group-flush">
                    <?php while($data = pg_fetch_object($datas)): ?>
                        ...
                        <br>No.pemesanan: <?=$data->no_pemesanan?>
                            <a class="btn btn-outline-success" href="detail-belumbayar-peternak.php?id=<?=$data->no_pemesanan?>&nama=<?=$data->name?>" style="float:right; role="button">Detail</a>
                        </li>
                    <?php endwhile; ?>   
                </ul>
